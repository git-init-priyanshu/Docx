...
export default function DocumentContent({ initialDocs, initialSession, quickStart }: Props) {
  const router = useRouter();
  const clientSession = useClientSession();

  // Seed from server-provided session to avoid a loading flash on first paint.
  // useEffect syncs once the client session resolves.
  const [session, setSession] = useState<Session | null>(initialSession);
  useEffect(() => {
    if (clientSession !== null) setSession(clientSession);
  }, [clientSession]);
  ...
  const { docs, isLoading: docsLoading } = useDocs(userId, initialDocs);

  // For authenticated users initialDocs is populated server-side → never show skeleton.
  // For guests, show skeleton while session resolves and localStorage docs load.
  const isLoading = session === null || (session?.id === undefined && docsLoading);
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto bg-[var(--lp-paper)]">
          <div className="max-w-[1180px] mx-auto px-6 md:px-8 py-8">
            ...
            {/* Quick start — rendered server-side, shown here via prop */}
            ...
            {folder === "all" && !isLoading && recent.length > 0 && (
              <section className="mb-10">
                <div className="flex items-end justify-between mb-3">
                  ...
                  <span className="font-mono text-[10.5px] uppercase tracking-wider text-[var(--lp-muted)]">
                    Recent
                  </span>
                </div>
            ...
            <section>
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="h-8 px-2.5 rounded-md border border-[var(--lp-border)] bg-[var(--lp-card)] text-[var(--lp-ink)] text-[12px] outline-none cursor-pointer"
                  >
                    <option value="recent">Recently updated</option>
                    <option value="alpha">A → Z</option>
                  </select>
                  <div className="flex border border-[var(--lp-border)] rounded-md overflow-hidden">
                    <button
                      onClick={() => setView("grid")}
                      className={`h-8 w-8 flex items-center justify-center transition text-[var(--lp-ink)] ${
                        view === "grid" ? "bg-[var(--lp-paper-2)]" : "bg-[var(--lp-card)]"
                      }`}
                      title="Grid view"
                    >
                    ...
                    <button
                      ...
                      className={`h-8 w-8 flex items-center justify-center transition text-[var(--lp-ink)] ${
                        view === "list" ? "bg-[var(--lp-paper-2)]" : "bg-[var(--lp-card)]"
                      }`}
                      title="List view"
                    >
              ...
              {/* List header */}
              {view === "list" && (
                <div className="grid grid-cols-[1fr,140px,90px,32px] gap-4 px-4 pb-2 font-mono uppercase tracking-wider text-[10px] text-[var(--lp-muted)]">
                  <span>Name</span>
                  <span>Owner</span>
                  <span>Updated</span>
                  <span />
              ...
              {!isLoading && filtered.length === 0 && (
                <div className="rounded-lg border border-[var(--lp-border)] bg-[var(--lp-card)] p-12 text-center">
                  <div className="text-[14px] font-medium mb-1 text-[var(--lp-ink)]">
